...
- [x] Task 1: 审计与补全 Orchestrator 事件循环集成 (AC: #1, #2, #3)
  - [x] 1.1 审计 `src/ato/core.py` 中 merge queue 的轮询集成：
  ...
  - [x] 1.2 审计 `src/ato/core.py` 与 `src/ato/transition_queue.py` 的职责边界：
  ...
  - [x] 1.3 补全 regression pass 后的完整收尾流程：
  ...
  - [x] 1.4 补全 regression fail 后的完整处理流程：
...
- [x] Task 2: Operator 决策处理流程（沿用现有 approval consumer） (AC: #4)
  - [x] 2.1 在 `src/ato/core.py::_handle_approval_decision()` 中补齐 `regression_failure` 分支：
  ...
  - [x] 2.2 `rebase_conflict` / `precommit_failure` 的决策路由继续留在同一个 approval consumer：
...
- [x] Task 3: Rebase 冲突与 Pre-commit 合同收敛 (AC: #5, #6)
  - [x] 3.1 审计 `_handle_rebase_conflict()` 实现完整性：
    - 确认创建 `rebase_conflict` approval 包含冲突文件列表
    - 确认操作者决策路由正确（manual_resolve / skip / abandon）
  - [x] 3.2 收敛 pre-commit hook 失败的真实范围（FR54）：
  ...
  - [x] 3.3 确认 rebase 冲突不触发 merge queue 冻结（仅 regression 失败冻结）

- [x] Task 4: 测试覆盖（优先扩展现有测试文件） (AC: #1-#6)
  - [x] 4.1 追加 `tests/unit/test_merge_queue.py`：
  ...
  - [x] 4.2 追加 `tests/unit/test_core.py`：
  ...
  - [x] 4.3 追加 `tests/integration/test_notification_flow.py` / `tests/unit/test_worktree_mgr.py`：
  ...
  - [x] 4.4 仅当现有测试承载不了真实跨模块路径时，才新增新的 integration 文件；默认先扩展 `test_merge_queue.py`、`test_core.py`、`test_notification_flow.py`

- [x] Task 5: 崩溃恢复场景验证 (AC: #1, #3)
  - [x] 5.1 审计 `merge_queue.recover_stale_lock()` 覆盖以下场景：
  ...
  - [x] 5.2 优先扩展现有 `tests/unit/test_merge_queue.py` 与 `tests/unit/test_recovery.py`：
...
- ✅ Task 1 审计确认：`core.py` 中 `merge_queue.process_next()`、`check_regression_completion()`、`recover_stale_lock()` 三个集成点均已正确调用。职责边界正确——approval 决策消费在 `Orchestrator._handle_approval_decision()`，状态转换在 `TransitionQueue`。
- ✅ Task 1 补全：`_handle_regression_failure()` payload 增加 `story_id` 和 `test_output_summary`（AC3），`_run_regression_test()` 失败时存储 stdout+stderr 到 `error_message`，`check_regression_completion()` 将 error_message 传递到 approval payload。
- ✅ Task 2 审计确认：`regression_failure` 三种决策（revert/fix_forward/pause）均已正确实现。revert 仅在成功后 unfreeze + cleanup；fix_forward 提交 `regression_fail` 事件回到 fixing 并保持 frozen；pause 仅记录日志。
- ✅ Task 3 审计确认：`_handle_rebase_conflict()` 创建 approval 包含冲突文件列表，不冻结 queue。`_handle_precommit_failure()` 作为 escalation 路径保留，当前 ff-only 路径不触发。
- ✅ Task 4 测试覆盖：追加 17 个新测试覆盖 happy path、regression failure payload、rebase conflict 不冻结、revert/fix_forward/pause 三种决策、通知单一出口、worktree revert 合同、崩溃恢复场景。
- ✅ Task 5 崩溃恢复：追加 3 个 crash recovery 测试——regression task 失败恢复、regression 结果未知冻结、merge 中断释放锁。
- ✅ Code Review 修复 #1：`recover_stale_lock()` 结果未知分支新增 `complete_merge(db, sid, success=False)` 将 entry 标记为 failed（quarantine），防止 `check_regression_completion()` 在 lingering task 写回结果后绕过人工决策 gate。
- ✅ Code Review 修复 #2：`_run_regression_test()` 失败时同时捕获 stdout 和 stderr（合并输出），修复 pytest 等框架将失败详情输出到 stdout 时摘要退化为通用 "Regression test failed" 的问题。
- ✅ Code Review 修复 #3：CLI `_format_approval_detail()` 新增 "失败摘要" 段落渲染 `test_output_summary` 字段，操作者在 `ato approval-detail` 中可直接看到 regression 失败详情用于 revert/fix_forward 决策。
- ✅ Code Review 修复 #4：`_extract_impact()` 对 `regression_failure` 改为读取 `blocked_count`（由 `_handle_regression_failure` 查询 waiting entries 写入），无 `blocked_count` 时显示 "merge queue 已冻结，所有后续 merge 被阻塞"，消除旧 `blocked_stories` 缺失时 "后续 0 个 merge 被阻塞" 的误导性文案。
- ✅ Code Review 修复 #5：`recover_stale_lock()` 的 "结果未知" 分支改为调用 `_handle_regression_failure()`，消除 crash recovery 路径与正常失败路径的 payload schema 分歧（缺少 story_id/blocked_count）。测试新增 payload 合同断言。
...
- 2026-03-27: dev-story 实现 —— 审计 6 个核心模块端到端集成、补全 regression failure payload（AC3）、追加 20 个测试覆盖 AC1-AC6 + 崩溃恢复场景。全量 1325 测试通过，0 回归。
- 2026-03-27: code review 修复 —— 修复 5 个 bug：(1) recover_stale_lock quarantine 防绕过 gate，(2) stdout+stderr 合并捕获，(3) CLI 渲染 test_output_summary，(4) _extract_impact regression_failure 影响范围文案，(5) crash recovery payload schema 统一。全量 1328 测试通过。

### File List

- `src/ato/merge_queue.py` — 补全 regression failure payload（story_id + test_output_summary），stdout+stderr 合并捕获，recover_stale_lock quarantine entry
- `src/ato/cli.py` — approval-detail 渲染 test_output_summary 失败摘要段落
- `tests/unit/test_merge_queue.py` — 追加 happy path / rebase 不冻结 / payload 内容 / stdout+stderr 捕获 / 崩溃恢复 quarantine 测试
- `tests/unit/test_core.py` — 追加 revert/fix_forward/pause 三种决策 + rebase 决策路由测试
- `tests/unit/test_worktree_mgr.py` — 追加 revert_merge_range git 合同验证测试
- `tests/unit/test_cli_notification.py` — 追加 regression_failure approval detail 渲染 test_output_summary 测试
- `tests/integration/test_notification_flow.py` — 追加 milestone 单一出口 + URGENT bell 自包含验证测试
