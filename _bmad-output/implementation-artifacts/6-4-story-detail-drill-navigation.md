...
- [x] Task 1: 增强第 1 层 Story 概览 (AC: #1)
  - [x] 1.1 修改 `DashboardScreen._render_story_detail()` 增加当前 findings 摘要
  - [x] 1.2 增加执行产物 / 执行历史的轻量计数摘要（不要把全量 detail rows 塞进 2s 轮询快照）
  - [x] 1.3 有 CL 数据的 story 显示简要收敛信息

- [x] Task 2: StoryDetailView 组件创建 (AC: #2, #5, #6)
  - [x] 2.1 将 `src/ato/tui/story_detail.py`（当前 stub）实现为完整的可聚焦 Widget / Container
  - [x] 2.2 实现状态流可视化（StoryPhaseFlow），顺序严格使用 `[“queued”, *CANONICAL_PHASES, “done”]`
  - [x] 2.3 实现 Findings 摘要区块（blocking/suggestion × open/closed）
  - [x] 2.4 实现执行产物区块：优先解析 `context_briefing.artifacts_produced`，fallback 到 `expected_artifact`；不得把控制标记伪装成”文件变更”
  - [x] 2.5 实现成本明细区块（使用 `cost_log` 的真实字段：`phase/role/cli_tool/model/input_tokens/output_tokens/cache_read_input_tokens/cost_usd`）
  - [x] 2.6 实现执行历史区块（使用 `tasks` 的真实字段：`phase/role/cli_tool/status/started_at/completed_at/duration_ms/context_briefing/expected_artifact`）
  - [x] 2.7 详情页绑定 `f/c/h/l/escape`，并在详情模式下禁用审批快捷键语义

- [x] Task 3: ConvergentLoopProgress Widget (AC: #4)
  - [x] 3.1 创建 `src/ato/tui/widgets/convergent_loop_progress.py`
  - [x] 3.2 实现轮次可视化（●/◐/○）+ 当前去重后的 findings 统计 + 收敛率 + 状态文字
  - [x] 3.3 在 StoryDetailView 中集成，仅对有 CL 数据的 story 显示

- [x] Task 4: DashboardScreen Enter / ESC 导航集成 (AC: #2, #5, #6)
  - [x] 4.1 在 `DashboardScreen` 增加 Enter 键绑定（`action_drill_in`）和 ESC 返回（`action_back`）
  - [x] 4.2 three-panel 模式：给 `#right-top-switcher` 新增 `StoryDetailView` 槽位并切换显示
  - [x] 4.3 tabbed 模式：在 `tab-stories` 内增加局部 list/detail 切换容器；不要 push 新 Screen，也不要替换整个 `TabbedContent`
  - [x] 4.4 保持 `ATOApp.action_switch_tab()` 的 `[1]-[4]` 契约不变
  - [x] 4.5 保持左面板选中状态和当前 focus 语义不变

- [x] Task 5: 第 2.5 层快捷键子视图展开 (AC: #3)
  - [x] 5.1 在 StoryDetailView 内实现 `f` / `c` / `h` 键绑定
  - [x] 5.2 `f` 展开 Findings 详细列表（DataTable / Tree / VerticalScroll 任选其一，但不要和现有 focus 语义冲突）
  - [x] 5.3 `c` 展开成本明细列表
  - [x] 5.4 `h` 展开执行历史时间轴
  - [x] 5.5 `l` 键显示 placeholder 提示（例如”日志查看将在后续版本提供”），不要尝试 `tail -f` 不存在的路径
  - [x] 5.6 ESC 从展开子视图返回详情页概览

- [x] Task 6: 数据查询扩展 (AC: #2, #3, #4)
  - [x] 6.1 保持 `ATOApp._load_data()` 的 2s 轮询轻量；Story detail 数据改为**按需**查询，不在每轮轮询中加载全量 tasks/cost_log/findings 明细
  - [x] 6.2 复用 `get_tasks_by_story()` 获取执行历史，并复用 Story 5.2 的 artifact 展示合同：`context_briefing.artifacts_produced` 优先，fallback `expected_artifact`
  - [x] 6.3 复用 `get_cost_logs_by_story()` 获取成本明细
  - [x] 6.4 复用 `get_findings_by_story()` 获取逐条 findings；当前状态摘要 / 收敛率复用 `get_story_findings_summary()` 或等价去重逻辑，避免跨轮重复累计
  - [x] 6.5 通过 `ATOApp → DashboardScreen → StoryDetailView` 推送结构化 detail snapshot；如需提炼共享 helper，应放在中性模块，不要让 TUI 反向 import `cli.py`

- [x] Task 7: TCSS 样式 (AC: #2, #4, #6)
  - [x] 7.1 StoryDetailView 布局样式（VerticalScroll + 区块间距）
  - [x] 7.2 StoryPhaseFlow 样式（phase 节点间距 + 高亮当前）
  - [x] 7.3 ConvergentLoopProgress 样式（进度条 + 收敛率颜色编码）
  - [x] 7.4 展开子视图样式（DataTable / Tree / Scroll 样式与全局主题一致）
  - [x] 7.5 three-panel / tabbed 模式下的 detail 样式都可用

- [x] Task 8: widgets 模块导出
  - [x] 8.1 在 `src/ato/tui/widgets/__init__.py` 导出 `ConvergentLoopProgress`

- [x] Task 9: 单元测试
  - [x] 9.1 StoryDetailView 渲染测试（各区块内容正确性）
  - [x] 9.2 StoryPhaseFlow 测试（实际 phase 顺序与当前阶段高亮正确）
  - [x] 9.3 ConvergentLoopProgress 测试（轮次可视化、去重后 findings 统计、收敛率计算）
  - [x] 9.4 Artifact 展示测试（`context_briefing.artifacts_produced` 优先，fallback `expected_artifact`）
  - [x] 9.5 成本明细展开内容测试（真实 `CostLogRecord` 字段）
  - [x] 9.6 执行历史展开内容测试（真实 `TaskRecord` 字段）

- [x] Task 10: 集成测试
  - [x] 10.1 Enter 键从主屏进入详情页（three-panel）
  - [x] 10.2 ESC 从详情页返回主屏（three-panel）
  - [x] 10.3 `f/c/h` 展开子视图 + ESC 返回
  - [x] 10.4 ConvergentLoopProgress 在有 CL 数据 story 上显示
  - [x] 10.5 tabbed 模式下 `tab-stories` 的 list/detail 切换生效
  - [x] 10.6 tabbed 模式下 `[1]-[4]` 仍切换 Tab，不被详情页实现破坏
  - [x] 10.7 导航后左面板选中状态保持
  - [x] 10.8 相关 TUI 回归通过
...
- 2026-03-28: dev-story 实现 —— 全部 10 个 Task (39 subtasks) 完成；38 新测试 + 1359 回归测试全部通过；ruff + mypy 通过
...
- Widget.Message → textual.message.Message: Textual API 中 Message 基类在 textual.message 模块，非 Widget 子属性

### Completion Notes List

- ✅ Task 1: 增强 `_render_story_detail()` 增加 findings 摘要（B↑/B↓/S↑/S↓）、task 计数、CL 收敛率
- ✅ Task 2: StoryDetailView 完整实现，含 phase flow、findings/artifacts/cost/history 区块
- ✅ Task 3: ConvergentLoopProgress widget 实现（●/◐/○ + 收敛率 + 状态）
- ✅ Task 4: Enter/ESC 导航集成（three-panel #right-top-switcher + tabbed #tab-stories-switcher）
- ✅ Task 5: f/c/h/l/ESC 键绑定，展开子视图 + l placeholder
- ✅ Task 6: ATOApp.load_story_detail() 按需查询，复用 db.py 现有 helpers
- ✅ Task 7: TCSS 样式：StoryDetailView / detail sections / ConvergentLoopProgress
- ✅ Task 8: widgets/__init__.py 导出 ConvergentLoopProgress
- ✅ Task 9: 22 单元测试（StoryDetailView + ConvergentLoopProgress + artifacts + cost/history）
- ✅ Task 10: 8 集成测试（Enter/ESC/f-c-h/CL/tabbed/digit-keys/selection/regression）
- 全量回归 1397 passed, 0 failed
- ruff check + ruff format + mypy 全部通过

### Implementation Plan

**导航策略：** 局部 ContentSwitcher（不使用 Screen push）
- three-panel: #right-top-switcher 新增 #right-top-detail (StoryDetailView)
- tabbed: tab-stories 内新增 #tab-stories-switcher 切换 list/detail
- 详情模式下 y/n/d 审批键被 guard 不消费

**数据流：** 轻轮询 + 按需详情
- _load_data() 增加 story_task_counts（轻量 GROUP BY 聚合）
- load_story_detail() 按需查询 tasks/cost_logs/findings
- StoryDetailView.update_detail() 接收结构化数据

**Convergent Loop:** CL 进度内联渲染在 StoryDetailView 中
- 复用 get_story_findings_summary() 去重语义
- still_open 视作 open

### File List

**新增文件：**
- src/ato/tui/story_detail.py — StoryDetailView 主实现（phase flow + 摘要 + 子视图展开）
- src/ato/tui/widgets/convergent_loop_progress.py — ConvergentLoopProgress Widget
- tests/unit/test_story_detail_view.py — StoryDetailView 单元测试（22 tests）
- tests/unit/test_convergent_loop_progress.py — CL 进度组件单元测试（9 tests）
- tests/integration/test_tui_story_detail.py — 钻入导航集成测试（8 tests）

**修改文件：**
- src/ato/tui/dashboard.py — Enter/ESC 绑定 + StoryDetailView 集成 + 增强概览 + 审批键 guard
- src/ato/tui/app.py — load_story_detail() + story_task_counts 轮询 + 传递到 DashboardScreen
- src/ato/tui/app.tcss — StoryDetailView / ConvergentLoopProgress / detail sections 样式
- src/ato/tui/widgets/__init__.py — 导出 ConvergentLoopProgress
- _bmad-output/implementation-artifacts/sprint-status.yaml — 6-4 状态更新
