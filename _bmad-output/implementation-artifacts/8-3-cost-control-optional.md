...
- [ ] Task 1: 配置模型与校验改造 (AC: #1, #2, #4)
  - [ ] 1.1 `src/ato/config.py` 中 `ATOSettings.cost: CostConfig = CostConfig()` 改为 `cost: CostConfig | None = None`
  - [ ] 1.2 保持 `CostConfig` 模型本身不变；显式 `cost` 配置仍沿用现有字段与默认值
  - [ ] 1.3 `src/ato/config.py::_validate_numeric_bounds()` 中对 cost 相关校验增加 `if config.cost is not None` 守卫

- [ ] Task 2: 恢复路径兼容 `cost=None` (AC: #3, #4)
  - [ ] 2.1 `src/ato/recovery.py` 中所有 `self._settings.cost.blocking_threshold` 读取点改为统一 helper 或显式守卫
  - [ ] 2.2 当 `settings is None` 或 `settings.cost is None` 时，blocking 阈值 fallback 为 `10`
  - [ ] 2.3 当 `settings.cost` 显式存在时，继续把配置中的 `blocking_threshold` 传给 `ConvergentLoop(...)` 与 `maybe_create_blocking_abnormal_approval(...)`
  - [ ] 2.4 不修改 `src/ato/core.py` 的 merge approval `cost_usd` 聚合逻辑；它继续仅基于 `cost_log` 数据工作

- [ ] Task 3: 更新配置模板 (AC: #5)
  - [ ] 3.1 `ato.yaml.example` 中将 `cost` 配置段整体注释掉
  - [ ] 3.2 注释说明“不配置时不做预算上限控制；如需自定义预算或 blocking 阈值，再取消注释”
  - [ ] 3.3 保留模板可读性与拷贝即用属性，不覆盖 Epic 8 其他 story 对同一模板文件的改动

- [ ] Task 4: 更新测试 (AC: #1-#4)
  - [ ] 4.1 `tests/unit/test_config.py` 增加/更新用例：缺少 `cost` 配置段时 `load_config()` 成功且 `config.cost is None`
  - [ ] 4.2 `tests/unit/test_config.py` 增加/更新用例：`cost: null` 与完全省略 `cost` 行为一致
  - [ ] 4.3 `tests/unit/test_config.py` 保留显式 `cost` 配置的正向断言，以及非法 `budget_per_story` / `blocking_threshold` 的失败断言
  - [ ] 4.4 `tests/unit/test_recovery.py` 增加用例：`settings.cost is None` 时恢复路径使用 fallback `10`
  - [ ] 4.5 `tests/unit/test_recovery.py` 增加用例：显式 `blocking_threshold` 时恢复路径传递配置值
