...
- [x] Task 1: 扩展配置模型并保持向后兼容 (AC: #1, #2)
  - [x] 1.1 在 `src/ato/config.py` 中为 `ATOSettings` 新增 `regression_test_commands` 字段；如需可变默认值，使用 Pydantic v2 推荐写法而非共享可变默认对象
  - [x] 1.2 保留现有 `regression_test_command: str = "uv run pytest"` 字段，避免破坏已有配置与测试
  - [x] 1.3 新增 `get_regression_commands() -> list[str]`：优先返回 `regression_test_commands`，否则回退为 `[regression_test_command]`

- [x] Task 2: 改造 regression runner，但保持现有 task / merge-queue 合同 (AC: #3, #4, #5)
  - [x] 2.1 保持 `src/ato/merge_queue.py::_dispatch_regression_test()` 与 `mark_regression_dispatched()` 的单 task 合同不变；不要新增 DB migration 或多 task 设计
  - [x] 2.2 将 `_run_regression_test()` 改为循环执行 `self._settings.get_regression_commands()`
  - [x] 2.3 每条命令都使用当前已有的 `shlex.split(cmd)`、`asyncio.create_subprocess_exec(..., cwd=repo_root)` 与三阶段清理协议
  - [x] 2.4 每条命令独立使用 `timeout.structured_job`；任一命令非 0 或超时立即中止后续命令
  - [x] 2.5 失败时 `error_message` 包含失败命令序号、命令文本和 stdout/stderr 摘要，并继续遵循 `<=1000` 字符截断
  - [x] 2.6 全部命令成功时，仅更新现有 regression task 为 `status="completed"`、`exit_code=0`；`check_regression_completion()` 与 approval payload 路径无需改 schema

- [x] Task 3: 更新配置模板，使用真实存在的测试层级示例 (AC: #6)
  - [x] 3.1 将 `ato.yaml.example` 的 regression 示例改为多命令格式：
    ```yaml
    ...
    ```
  - [x] 3.2 保留 `regression_test_command` 的注释说明，明确其仅用于向后兼容旧配置

- [x] Task 4: 扩展现有测试，而不是新建平行测试矩阵 (AC: #1-#6)
  - [x] 4.1 扩展 `tests/unit/test_config.py`：覆盖多命令加载顺序、plural 优先于 singular、仅 singular 时的 fallback
  - [x] 4.2 扩展 `tests/unit/test_merge_queue.py`：覆盖多命令全部成功、第二条失败即 short-circuit、每条命令独立 `shlex.split()`、仅 singular 时 fallback
  - [x] 4.3 如模板断言需要更新，优先在现有 `test_config.py` / `test_config_workflow.py` 中修正；不要为本 story 发明 `tests/e2e/` 目录
...
- ✅ 在 `ATOSettings` 新增 `regression_test_commands: list[str] | None = None` 字段（Pydantic v2，None 默认值避免可变默认问题）
- ✅ 新增 `get_regression_commands()` 方法：plural 优先，fallback 到 singular
- ✅ `_run_regression_test()` 改为循环执行命令链，保持单 task 记录合同
- ✅ 每条命令独立 `shlex.split()` + 独立 `timeout.structured_job` 超时
- ✅ 失败即中止：error_message 包含 1-based 序号、命令文本、stdout/stderr 摘要（<=1000 字符）
- ✅ `_dispatch_regression_test()`、`check_regression_completion()`、approval payload 路径不变
- ✅ `ato.yaml.example` 更新为多命令格式，保留 singular 注释说明
- ✅ 新增 5 个配置测试 + 6 个 regression runner 测试，全部测试 1457 passed
...
- 2026-03-28: dev-story 实现 —— 新增 plural 配置字段、get_regression_commands() helper、多命令顺序执行 runner、更新配置模板、11 个新测试全部通过

### File List

- `src/ato/config.py` — 新增 `regression_test_commands` 字段和 `get_regression_commands()` 方法
- `src/ato/merge_queue.py` — `_run_regression_test()` 改为多命令顺序执行
- `ato.yaml.example` — regression 配置更新为多命令格式
- `tests/unit/test_config.py` — 新增 `TestRegressionMultiCommand` 测试类（5 个测试）
- `tests/unit/test_merge_queue.py` — 新增 `TestMultiCommandRegression` 测试类（6 个测试）+ 现有 mock 适配
