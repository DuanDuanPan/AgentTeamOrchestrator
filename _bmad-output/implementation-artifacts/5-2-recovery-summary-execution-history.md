...
- [ ] Task 1: 恢复摘要渲染器 (AC: #1, #2, #5)
  - [ ] 1.1 在 `src/ato/recovery_summary.py` 新建模块，实现 `render_recovery_summary(result: RecoveryResult, db_path: Path, console: Console | None = None) -> None`
  - [ ] 1.2 渲染首行："✔ 数据完整性检查通过"（Rich `green` 样式，不使用 Textual TCSS 变量名）
  - [ ] 1.3 渲染恢复模式标识：
  ...
  - [ ] 1.5 渲染 needs_human 决策列表（使用 `rich.Table`）：
  ...
  - [ ] 1.6 所有输出写到 stderr（通过 `Console(stderr=True)` 或直接 `sys.stderr`）
  - [ ] 1.7 渲染使用 `rich.panel.Panel` 包裹（标题："恢复摘要"），边框色为 `green`（无 needs_human）或 `yellow`（有 needs_human）

- [ ] Task 2: core.py 集成恢复摘要输出 (AC: #5)
  - [ ] 2.1 在 `src/ato/core.py` 的 `_detect_recovery_mode()` 返回 RecoveryResult 后调用 `render_recovery_summary(result, self._db_path)`
  - [ ] 2.2 确保渲染在 RecoveryEngine 完成后、orchestrator 事件循环启动前执行
  - [ ] 2.3 保留 `RecoveryEngine.run_recovery()` 现有 `recovery_complete` 作为恢复逻辑日志；若额外记录 `recovery_summary_rendered`，只记录渲染成功与行数，不重复分类逻辑

- [ ] Task 3: ato history 命令 (AC: #3)
  - [ ] 3.1 在 `src/ato/cli.py` 新增 `@app.command("history")` 命令：
  ...
  - [ ] 3.4 表格底部追加汇总行：总耗时、总成本、任务数
  - [ ] 3.5 story 不存在时使用 `_format_cli_error()` 输出错误（复用 Story 4-4 规范）

- [ ] Task 4: ato cost report 命令 (AC: #4)
  - [ ] 4.1 在 `src/ato/cli.py` 新增 `cost_app = typer.Typer(...)` 并 `app.add_typer(cost_app, name="cost")`：
  ...
  - [ ] 4.3 Story 详情模式（`--story <id>`）：
    ...
    - 底部汇总行
  - [ ] 4.4 无成本数据时显示友好提示（"暂无成本数据。运行 story 后将自动记录。"）

- [ ] Task 5: DB Helper 函数扩展 (AC: #3, #4)
  - [ ] 5.1 在 `src/ato/models/db.py` 新增：
  ...
  - [ ] 5.2 将 `get_tasks_by_story()` 改为稳定的 started_at 排序（`started_at` 为 NULL 的记录放最后，tie-breaker 用 `rowid`）

- [ ] Task 6: 单元测试 (AC: #1-#4)
  - [ ] 6.1 `tests/unit/test_recovery_summary.py`（新建文件）：
  ...
  - [ ] 6.2 `tests/unit/test_cli_history.py`（新建文件）：
  ...
  - [ ] 6.3 `tests/unit/test_cli_cost.py`（新建文件）：
...
- [ ] Task 7: 集成测试 (AC: #1, #3, #4, #5)
  - [ ] 7.1 `tests/integration/test_recovery_summary_e2e.py`（新建文件）：
  ...
  - [ ] 7.2 `tests/integration/test_history_cost_e2e.py`（新建文件）：
