...
- [ ] Task 1: 实现 Orchestrator 核心类 (AC: #1, #3, #4, #6)
  - [ ] 1.1 在 `src/ato/core.py` 中实现 `Orchestrator` 类，构造函数接收 `settings: ATOSettings` 和 `db_path: Path`
  - [ ] 1.2 实现 `async run() -> None`——主事件循环：写 PID → 初始化组件 → 恢复检测 → 轮询循环 → 清理
  - [ ] 1.3 实现轮询循环：`await nudge.wait(timeout=polling_interval)` 替代固定 sleep，每轮检测 transition 事件
  - [ ] 1.4 实现 `async _startup() -> None`——消费已解析的 `settings`、初始化 TransitionQueue、注册 SIGTERM/SIGUSR1 handler、写 PID 文件
  - [ ] 1.5 实现 `async _shutdown() -> None`——标记 running tasks 为 paused、停止 TransitionQueue、删除 PID 文件
  - [ ] 1.6 注册 SIGUSR1 信号 handler，将信号转为 `nudge.notify()`（使用 `loop.add_signal_handler()`）
  - [ ] 1.7 实现恢复检测日志：扫描 tasks 表 running/paused 状态，structlog 输出恢复模式信息

- [ ] Task 2: 实现 PID 文件管理 (AC: #1, #2, #5)
  - [ ] 2.1 实现 `write_pid_file(pid_path: Path) -> None`——写入当前进程 PID
  - [ ] 2.2 实现 `read_pid_file(pid_path: Path) -> int | None`——读取 PID，文件不存在返回 None
  - [ ] 2.3 实现 `is_orchestrator_running(pid_path: Path) -> bool`——读取 PID + `os.kill(pid, 0)` 检测存活；stale PID 视为未运行
  - [ ] 2.4 实现 `remove_pid_file(pid_path: Path) -> None`——删除 PID 文件（幂等）

- [ ] Task 3: 实现 `ato start` CLI 命令 (AC: #1, #5)
  - [ ] 3.1 在 `src/ato/cli.py` 中添加 `ato start` 命令
  - [ ] 3.2 接受可选参数：`--db-path`、`--config`（ato.yaml 路径）
  - [ ] 3.3 启动前调用 `is_orchestrator_running()` 检测重复启动
  - [ ] 3.4 调用 `configure_logging()` 初始化日志（使 preflight / orchestrator 启动日志可见）
  - [ ] 3.5 调用 `run_preflight(project_path=Path.cwd(), db_path=resolved_db, include_auth=False)` 做快速 preflight；存在 HALT 时以环境错误退出
  - [ ] 3.6 调用 `load_config()` 解析配置，并将 `ATOSettings` 注入 `Orchestrator`；不要在 `core.py` 二次读取 YAML
  - [ ] 3.7 创建 Orchestrator 实例并 `asyncio.run(orchestrator.run())`

- [ ] Task 4: 实现 `ato stop` CLI 命令 (AC: #2)
  - [ ] 4.1 在 `src/ato/cli.py` 中添加 `ato stop` 命令
  - [ ] 4.2 接受可选参数：`--pid-file`（默认 `.ato/orchestrator.pid`）
  - [ ] 4.3 读取 PID 文件，发送 SIGTERM 到 Orchestrator 进程
  - [ ] 4.4 轮询 `os.kill(pid, 0)` 等待进程退出；超时后给出提示，必要时升级为 `SIGKILL` 清理
  - [ ] 4.5 PID 文件不存在或进程不存活时输出友好提示

- [ ] Task 5: 单元测试——Orchestrator 核心行为 (AC: #1, #3, #4, #5, #6)
  - [ ] 5.1 创建 `tests/unit/test_core.py`
  - [ ] 5.2 测试 PID 文件写入/读取/删除/存活检测
  - [ ] 5.3 测试重复启动防护（PID 存在 + 进程存活 → 拒绝）
  - [ ] 5.4 测试恢复模式检测日志（running tasks → 崩溃恢复日志、paused tasks → 正常恢复日志、无 tasks → 全新启动日志）
  - [ ] 5.5 测试 SIGUSR1 handler 注册与 nudge 触发（mock loop.add_signal_handler）
  - [ ] 5.6 测试 shutdown 标记 running tasks 为 paused

- [ ] Task 6: 集成测试——启停端到端 (AC: #1, #2, #3)
  - [ ] 6.1 创建 `tests/integration/test_orchestrator_lifecycle.py`
  - [ ] 6.2 测试完整启动→轮询→停止流程（使用短轮询间隔 + 快速触发停止）
  - [ ] 6.3 测试启动后 PID 文件存在，停止后 PID 文件删除
  - [ ] 6.4 测试启动延迟 ≤3 秒（NFR5）

- [ ] Task 7: CLI 命令测试 (AC: #1, #2, #5)
  - [ ] 7.1 创建 `tests/unit/test_cli_start_stop.py`
  - [ ] 7.2 使用 `typer.testing.CliRunner` 测试 `ato start` 重复启动拒绝（exit code 1）
  - [ ] 7.3 测试 `ato stop` PID 不存在时的友好提示

- [ ] Task 8: 代码质量验证
  - [ ] 8.1 `uv run ruff check src/ato/core.py src/ato/cli.py` — 通过
  - [ ] 8.2 `uv run mypy src/ato/core.py src/ato/cli.py` — 通过
  - [ ] 8.3 `uv run pytest tests/unit/test_core.py tests/unit/test_cli_start_stop.py tests/integration/test_orchestrator_lifecycle.py -v` — 全部通过
  - [ ] 8.4 `uv run pytest` — 确认零回归（2026-03-24 本地 collect-only 基线为 454 tests）
