...
- [ ] Task 1: 创建 JSON Schema 定义文件 (AC: #2)
  - [ ] 1.1 创建 `schemas/` 目录
  - [ ] 1.2 创建 `schemas/review-findings.json`——review findings 输出的 JSON Schema：
  ...
  - [ ] 1.3 创建 `schemas/story-validation.json`——story artifact 结构验证 Schema（验证 story 产出格式是否合规）
  - [ ] 1.4 创建 `schemas/finding-verification.json`——单个 finding 验证结果的 Schema（用于 re-review 场景中 finding 闭合验证）

- [ ] Task 2: 实现 DeterministicValidator (AC: #1, #2)
  - [ ] 2.1 创建 `src/ato/validation.py` 模块
  - [ ] 2.2 实现 `load_schema(schema_name: str) -> dict[str, Any]`：从仓库根 `schemas/` 目录加载 JSON Schema 文件；当前 `pyproject.toml` 只打包 `src/ato`，因此不要对根目录 `schemas/` 使用 `importlib.resources`；schema 加载失败时抛出 `ConfigError`
  - [ ] 2.3 实现 `validate_artifact(artifact_data: dict[str, Any], schema_name: str) -> ValidationResult`：使用 `Draft202012Validator.check_schema()` + `Draft202012Validator(schema).iter_errors()` 收集完整错误列表；返回 `ValidationResult(passed: bool, errors: list[SchemaValidationIssue])`
  - [ ] 2.4 定义 `ValidationResult` 和 `SchemaValidationIssue` Pydantic 模型（放在 `schemas.py` 中）；不要命名为 `ValidationError`，避免与 `pydantic.ValidationError` 冲突
  - [ ] 2.5 确保全部验证路径耗时 ≤1 秒（NFR4）——JSON Schema 验证是纯 CPU 操作，预期 <10ms

- [ ] Task 3: 定义 FindingRecord 模型与 findings 表 (AC: #3)
  - [ ] 3.1 在 `src/ato/models/schemas.py` 中定义：
  ...
  - [ ] 3.2 实现 `compute_dedup_hash(file_path: str, rule_id: str, severity: str, description: str) -> str`：
  ...
  - [ ] 3.3 在 `src/ato/models/db.py` 中添加 `_FINDINGS_DDL`：
    ```sql
    ...
    ```
  - [ ] 3.4 添加索引：`CREATE INDEX IF NOT EXISTS idx_findings_story_round ON findings(story_id, round_num)`
  - [ ] 3.5 添加索引：`CREATE INDEX IF NOT EXISTS idx_findings_dedup ON findings(dedup_hash)`
  - [ ] 3.6 在 `src/ato/models/migrations.py` 中添加 v4→v5 迁移（创建 findings 表 + 索引），更新 `SCHEMA_VERSION = 5`

- [ ] Task 4: 实现 findings CRUD 辅助函数 (AC: #3)
  - [ ] 4.1 在 `src/ato/models/db.py` 中添加 `insert_finding(db, record: FindingRecord) -> None`：`model_validate()` 后写入
  - [ ] 4.2 添加 `insert_findings_batch(db, records: list[FindingRecord]) -> None`：批量插入，事务内执行
  - [ ] 4.3 添加 `get_findings_by_story(db, story_id: str, *, round_num: int | None = None) -> list[FindingRecord]`
  - [ ] 4.4 添加 `get_open_findings(db, story_id: str) -> list[FindingRecord]`：查询 `status IN ('open', 'still_open')` 的 findings
  - [ ] 4.5 添加 `update_finding_status(db, finding_id: str, new_status: FindingStatus) -> None`
  - [ ] 4.6 添加 `count_findings_by_severity(db, story_id: str, round_num: int) -> dict[str, int]`：返回 `{"blocking": N, "suggestion": M}`

- [ ] Task 5: 实现 blocking 阈值 escalation (AC: #4)
  - [ ] 5.1 在 `src/ato/validation.py` 中添加 `count_blocking_findings(db, story_id: str, round_num: int) -> int`：统计当前轮次 blocking finding 数量
  - [ ] 5.2 添加 `maybe_create_blocking_abnormal_approval(...) -> bool`：当 blocking 数量 `> threshold` 时调用 `db.py` 已有的 `insert_approval()` 创建 `approval_type="blocking_abnormal"` 记录，payload 包含 `{"blocking_count": N, "threshold": M, "round_num": R}`
  - [ ] 5.3 不在 `validation.py` 内硬编码全局 nudge 单例；由调用方按上下文传入 `Nudge` 或 `orchestrator_pid`，进程外路径使用 `send_external_nudge()`

- [ ] Task 6: 添加 jsonschema 依赖 (AC: #2)
  - [ ] 6.1 `uv add jsonschema`——JSON Schema 验证库（Python 标准生态，无额外系统依赖）

- [ ] Task 7: 测试 (AC: #1-#4)
  - [ ] 7.1 创建 `tests/fixtures/findings_valid.json`——通过 `review-findings.json` Schema 的样本数据
  - [ ] 7.2 创建 `tests/fixtures/findings_invalid.json`——缺少必需字段、severity 非法值等反例
  - [ ] 7.3 创建 `tests/unit/test_validation.py`：
  ...
  - [ ] 7.4 在 `tests/unit/test_schemas.py` 中补充：
  ...
  - [ ] 7.5 在 `tests/unit/test_db.py` 中补充：
  ...
  - [ ] 7.6 在 `tests/unit/test_migrations.py` 中补充：
